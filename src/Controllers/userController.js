...
exports.funcWatched = async (req, res) => {
  let driver;
  try {
    driver = await connectDB();
    const session = driver.session();
    ...
    return res.status(200).send({ message: "Pelicula marcada como vista." });
  } catch (error) {
    console.error(error); // Es recomendable registrar el error
    return res.status(500).send({ message: "Error marcando la pelicula como vista." });
  } finally {
    if (driver) {
      await driver.close();
    }
