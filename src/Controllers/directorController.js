...
//Funcion para crear un director
exports.createDirector = async (req, res) =>{
    ...
    try {
        ...
        const result = await session.run(
            `CREATE (:Director {birth_date: $birth_date, name: $name, nationality: $nationality})`,
            { birth_date, name, nationality}
        );

        return res.status(200).json({ message: "Director registrado correctamente"});
